...
interface QuoteSummaryPanelProps {
  accountInfo: { name: string; identificationNumber?: string };
  subtotal: number;
  tax: number;
      ...
      <Box className="px-5 py-4 flex flex-col gap-4">
        ...
        <Box className="flex flex-col gap-2">
          ...
          {commercialDiscountAmt > 0 && (
            <SummaryRow label="Desc. comercial" value={commercialDiscountAmt} negative />
          )}
          {otherDiscountAmt > 0 && (
            <SummaryRow label="Otros descuentos" value={otherDiscountAmt} negative />
          )}
          {retefuenteAmt > 0 && (
            <SummaryRow label="Retefuente" value={retefuenteAmt} negative />
          )}
        ...
        <button
          ...
          className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
